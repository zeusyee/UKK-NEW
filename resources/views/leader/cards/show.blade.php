        ...
        <div class="mt-6">
            ...
            @if($card->assignedUser)
                <div class="flex items-center bg-blue-50 px-4 py-3 rounded-lg">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-900">{{ $card->assignedUser->full_name }}</p>
                        <p class="text-xs text-gray-600">{{ $card->assignedUser->email }}</p>
                    </div>
                    <div class="ml-auto">
                        <span class="px-3 py-1 text-xs rounded-full {{ $card->assignment_status === 'completed' ? 'bg-green-100 text-green-800' : ($card->assignment_status === 'in_progress' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800') }}">
                            {{ ucfirst(str_replace('_', ' ', $card->assignment_status ?? 'not assigned')) }}
                        </span>
                    </div>
                </div>
            @else
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="text-sm text-gray-600">
                <i class="fas fa-info-circle mr-1"></i>Member creates and manages their own subtasks
            </div>
        </div>
        ...
        @if($card->subtasks->isEmpty())
            <div class="text-center py-12">
                ...
                <p class="text-gray-400 text-sm mt-2">The assigned member will create subtasks for this card.</p>
            </div>
        @else
            <div class="space-y-3">
